...
CSDKeliLsCommonUdp::CSDKeliLsCommonUdp(const std::string &hostname, const std::string &port, int &timelimit, CParserBase *parser, ros::NodeHandle &nh, ros::NodeHandle &pnh) :
    CSDKeliLsCommon(parser, nh, pnh),
    mHostName(hostname),
    mPort(port),
...
int CSDKeliLsCommonUdp::SendDeviceReq(const char *req, std::vector<unsigned char> *resp)
{
    if(mSocket == -1)
    {
...
int CSDKeliLsCommonUdp::GetDataGram(unsigned char *receiveBuffer, int bufferSize, int *length)
{
    int len;
    struct timeval tv;
    socklen_t addrlen;
